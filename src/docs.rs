...
pub static SHEETS: &[Entry] = &[
    ...
    doc!("sheet-recalc", "(sheet-recalc name) → list", "Reruns every formula — for ones that read the database or the clock, and for another sheet that wasn't open at the time, none of which are tracked.",
         "(sheet-recalc \"Budget\")"),
    doc!("sheet-format", "(sheet-format name area fmt) → list", "Merges a format into every cell of an area — a key set to nil is removed, and a nil format clears it. Given rows of formats instead, sets each cell's exactly from the area's corner. The keys are the host's: :num, :dp, :cur, :date, :bold, :italic, :align, :wrap, :bg, :fg, :border.",
    ...
    doc!("sheet-row-height", "(sheet-row-height name row height) → nil", "A row's height, or nil for the default. Rows are numbered from 1.", "(sheet-row-height \"Budget\" 3 48)"),
